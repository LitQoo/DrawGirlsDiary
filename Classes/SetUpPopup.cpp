...
SetUpPopup* SetUpPopup::create(int t_touch_priority, function<void()> t_end_func)
{
	SetUpPopup* t_mup = new SetUpPopup();
	t_mup->myInit(t_touch_priority, t_end_func);
	t_mup->autorelease();
	return t_mup;
}

void SetUpPopup::myInit(int t_touch_priority, function<void()> t_end_func)
{
	...
	title_str = myLoc->getLocalForKey(kMyLocalKey_setupTitle);
	content_str = myLoc->getLocalForKey(kMyLocalKey_setupContent);
	...
	KSLabelTTF* rightnow_label = KSLabelTTF::create(myLoc->getLocalForKey(kMyLocalKey_setupButton), mySGD->getFont().c_str(), 13);
	...
	rightnow_button->addTargetWithActionForControlEvents(this, cccontrol_selector(SetUpPopup::buttonAction), CCControlEventTouchUpInside);
	rightnow_button->setPreferredSize(CCSizeMake(131,44));
	rightnow_button->setPosition(ccp(0,-85));
...
void SetUpPopup::buttonAction(CCObject* sender, CCControlEvent t_event)
{
	if(!is_menu_enable)
		return;
	...
	myHSP->command("getcommonsetting", Json::Value(), json_selector(this, SetUpPopup::resultGetCommonSetting));
}

void SetUpPopup::resultGetCommonSetting(Json::Value result_data)
{
