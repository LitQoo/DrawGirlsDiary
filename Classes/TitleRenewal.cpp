...
void TitleRenewalScene::cardDataWrite()
{
	if(card_data_write_frame >= card_data_list.size())
	{
		for(int j=0;j<save_card_version_list.size();j++)
		{
			NSDS_SI(kSDS_CI_int1_version_i, save_card_version_list[j].x, save_card_version_list[j].y, false);
		}
		...
		unschedule(schedule_selector(TitleRenewalScene::cardDataWrite));
		return;
	}
	
	Json::Value cards = card_data_list;
...
void TitleRenewalScene::resultLoadedCardData( Json::Value result_data )
{
	...
	if(result_data["result"]["code"].asInt() == GDSUCCESS)
	{
		...
		card_data_list = result_data["list"];
		card_data_write_frame = 0;
		save_card_version_list.clear();
		
		schedule(schedule_selector(TitleRenewalScene::cardDataWrite));
